...
const Client: React.FC<ClientProps> = ({ id, name, slug, removeUser }) => {
  ...
  const editor = useMemo(() => {
    const editor = withCursor(
      withYjs(
        withLinks(withReact(withHistory(createEditor() as ReactEditor))),  // as CustomEditor
        sharedType
      ),
      provider.awareness
    );
  ...
  useEffect(() => {
    ...
    // In prod, when using y-websocket, provide the
    // initial state from the server.
    provider.on("sync", (isSynced: boolean) => {
      if (isSynced && sharedType.length === 0) {
        toSharedType(sharedType, [
          {type: "page", children: [{ type: "paragraph", children: [{ text: "start writing" }] }]},  
        ]);
      }
    });
